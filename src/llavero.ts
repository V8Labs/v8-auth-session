...
export function crearLlavero(opts: LlaveroOpts): Llavero {
  ...
  const ahora: Reloj = opts.ahora ?? (() => Date.now());
  ...
  const enV8 =
    typeof window !== 'undefined' && window.location?.hostname?.endsWith('v8labs.co');
  const cookieDomain =
    opts.cookieDomain === undefined ? (enV8 ? '.v8labs.co' : null) : opts.cookieDomain;
  ...
  function clearCookie(name: string) {
    try { document.cookie = `${name}=${cookieAttrs(0)}`; } catch { /* sin cookies */ }
  }
  function escribirCookie(name: string, value: string, maxAge: number) {
    try { document.cookie = `${name}=${value}${cookieAttrs(maxAge)}`; } catch { /* sin cookies */ }
  }
  ...
  function decodificar(raw: string | null): string | null {
  ...
  function leerDeCookie(key: string): string | null {
  ...
  function esVeredicto(status: number | null): boolean {
    if (status === null) return false;       // no hubo respuesta: red, DNS, TLS, timeout
    if (status === 408 || status === 429) return false; // timeout / rate-limit: no es del token
    if (status >= 500) return false;         // el servidor se cayó, no juzgó nada
    return true;                             // 2xx/4xx: GoTrue se pronunció
  }

  function marcarResultadoRefresh(status: number | null): void {
   ...
   * mucho UN conteo.
   */
  function tomarVeredicto(): boolean {
  ...
  function esRefreshDeSesion(url: string): boolean {
    return url.includes('/auth/v1/token');
  }

  const fetchConVeredicto: typeof fetch = async (input, init) => {
    const url =
      typeof input === 'string' ? input
      : input instanceof URL ? input.href
      : (input as Request).url;

    if (!esRefreshDeSesion(url)) return fetch(input, init);
  ...
  function identidadSesion(raw: string | null): string | null {
    if (!raw) return null;
    try {
      const s = JSON.parse(raw) as { expires_at?: number; user?: { id?: string } };
  ...
  function expiraEn(raw: string | null): number {
    if (!raw) return -1;
    try {
      const s = JSON.parse(raw) as { expires_at?: number };
  ...
  function sessionIdDe(raw: string | null): string | null {
    if (!raw) return null;
    try {
      const jwt = (JSON.parse(raw) as { access_token?: string })?.access_token;
      ...
      const claims = JSON.parse(
        atob(b64 + '=='.slice(0, (4 - (b64.length % 4)) % 4)),
  ...
  function sellarNacimiento(raw: string): void {
    ...
    let prev: { sid?: string; t?: number } | null = null;
    try {
      const raw2 = decodificar(readCookie(NACIDA_KEY));
      prev = raw2 ? (JSON.parse(raw2) as { sid?: string; t?: number }) : null;
    } catch { prev = null; }
    if (prev?.sid === sid && typeof prev?.t === 'number') return; // ya sellada
    try {
   ...
   * del servidor.
   */
  function edadDeSesion(): EdadSesion | null {
    try {
      ...
      if (dias < 0) return null; // reloj movido hacia atrás: no inventamos nada
  ...
  function contarFallo(id: string): number {
    try {
      const prevRaw = decodificar(readCookie(FALLOS_KEY));
      const prev = prevRaw ? (JSON.parse(prevRaw) as { id?: string; n?: number }) : null;
      ...
      if (readCookie(FALLOS_KEY) === null) return MAX_FALLOS;
      return n;
    } catch {
  ...
  function limpiarFallos(): void {
    try { clearCookie(FALLOS_KEY); } catch { /* ignore */ }
  }
  ...
  async function conSalidaExplicita(fn: () => Promise<unknown>): Promise<void> {
    salidas++;
    try {
      await fn();
    } finally {
  ...
  function limpiarTodo(): void {
    try { clearSharedCookie(KEY); } catch { /* ya no puede romper nada */ }
    limpiarFallos();
    ...
    if (HINT_KEY) clearCookie(HINT_KEY);
  }
  ...
  const storage: Almacen = {
    getItem: (key: string): string | null => {
      ...
      try { ls = window.localStorage.getItem(key); } catch { /* ignore */ }
      ...
      if (ls !== null) {
        ...
        if (readCookie(key) === null) {
          try { writeChunkedCookies(key, ls); } catch { /* ignore */ }
          return ls;
        }
        ...
        if (desdeCookie !== null && expiraEn(desdeCookie) > expiraEn(ls)) {
          try { window.localStorage.setItem(key, desdeCookie); } catch { /* ignore */ }
          return desdeCookie;
        }
        return ls;
      }
    ...
    removeItem: (key: string): void => {
      ...
      if (salidas > 0) { clearSharedCookie(key); limpiarFallos(); return; }
      ...
      if (enCookie === null) return;                        // nada que desalojar
      const id = identidadSesion(enCookie);
      if (id === null) { clearSharedCookie(key); return; }  // ilegible = inservible
